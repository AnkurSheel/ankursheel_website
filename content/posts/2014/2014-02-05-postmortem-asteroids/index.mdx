...
-   **Custom Model Format and ObjFormatter:** A custom model format meant that I could optimize my code to get the information I needed upfront without having to run multiple passes. Using the ObjFormatter to pre-process the model file before even starting the game, helped in improving the loading time.

-   **Collision Detection:** The collision detection algorithm was improved considerably.

-   **Pausing:** Instead of quitting the game directly on the game ending or the player pressing escape, the options screen would come up which allowed the player to restart or go to the main menu.
...
-   **No Screen Manager:** I would have loved to handle a screen manager to take care of all the transitions. With the addition of each screen, the amount of care needed to handle the transitions increased exponentially.
