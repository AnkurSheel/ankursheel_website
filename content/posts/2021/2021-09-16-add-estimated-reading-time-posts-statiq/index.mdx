...
excerpt: 'A guide to create a module to add estimated reading time as metadata for posts in websites created with Statiq'
tags:
    - 'statiq'
    - 'tutorial'
    - 'snippet'
featuredImage: './cover.png'
featuredImagePosition: 'bottom 10% center'
---

In this tutorial, I will be showing how to use the reading time calculated in [How to calculate reading time](./calculate-reading-time) to use in a Statiq module so that we can add reading time for our posts.
...
-   **_Line 10-11_**: Add a new metadata item with key _Reading_time_and a value obtained from a call to `GetReadingTime`. We use a_ReadingTimeWordsPerMinute_ setting to override the default value of 200 words per minute.
