...
As part of the [migration from Gatsby to Statiq](./migrating-gatsby-statiq), I want to deploy my website built with Statiq and Tailwind to Netlify. Netlify allows for only one build command on deployment. The issue is that Tailwind is available as an npm package while static is a .Net solution, and hence both require different commands to run.
