...
-   **_Line 2_:**: Use a **_for_each_** argument to iterate over the documents returned by the **_fileset_** function. **_for_each_** identifies each instance of the resource by its S3 path, making it easy to add/remove files. The fileset function enumerates over a set of filenames for a given path. It uses **_\*\*_** as the pattern for a recursive search.
