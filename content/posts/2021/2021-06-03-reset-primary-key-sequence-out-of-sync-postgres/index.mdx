---
title: 'How to reset the primary key sequence when it falls out of sync in Postgres?'
excerpt: 'In this post, I look at how we can fix the "duplicate key value violates unique constraint" error because the sequence is out of sync from the records in the table.'
tags:
    - 'postgres'
    - 'sql'
    - 'snippet'
...
When the **_Current Value_** is less than **_Max Value_**, our sequence is out-of-sync.
...
This will set the sequence to the next available value that is higher than any existing value for "**_id_**" in **_table_**. Further inserts will not result in a "duplicate key value violates unique constraint" error.
