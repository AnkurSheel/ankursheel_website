...
excerpt: "'Using an anti-forgery token is a pretty common way of securing your website from XSRF(Cross-Site Request Forgery) attacks. However, it does make it difficult to use Postman to test your ajax endpoints. How do you test the endpoints? Let's find out'"
...
I couldn't stop thinking that there has to be a better way to do this. Surely, I was not the only one who would want to do this. So, I did what any good engineer does - Open my browser and search stack overflow and google. I found a few articles but none of them worked for me.
...
-   **_Line 2_**: We send a request to the login page and we designate the callback function which receives 2 arguments - an error(if any) and a response.
