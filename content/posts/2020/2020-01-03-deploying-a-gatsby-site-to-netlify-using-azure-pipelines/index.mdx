...
excerpt: 'Deploying a gatsby site to Netlify has a few limits which resulted in my builds timing out. By using Azure Pipelines, the limits are no longer a problem'
tags:
    - 'devops'
    - 'tutorial'
    - 'Gatsby'
...
Azure Pipelines uses a YAML config file to build your project. We will use templates to have separate workflows for _pr_, and _release_ builds. Let's create 3 files:
...
I have separate workflows for Live and PR builds because I do not want PR builds to deploy to the live site.
...
1. **_parameters_**: The parameters to be passed to the job template
    1. **_deployScriptParams_**: For live builds, I pass the --prod flag, which tells Netlify to deploy the build to production.
