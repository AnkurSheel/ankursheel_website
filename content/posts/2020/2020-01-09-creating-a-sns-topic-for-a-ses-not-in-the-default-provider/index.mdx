...
Lets assume that most of our infrastructure is in _ap-southeast-2 (Sydney)_. But, our Amazon Simple Email Service(SES)
is setup in _us-west-2 (Oregon)_. We also want to create an Amazon Simple Notification Service(SNS) topics and Amazon
Simple Queue Service (SQS) queues so that we can handle bounce notifications. Now since SES is in a different region our
SNS topics also need to be in that region. So how do we tell terraform to create the topics in the SES region?
