---
title: 'How to handle Bounces in AWS SES using terraform: Part 1'
...
Recently, I had to work on a task to notify us in slack whenever an email that was sent through Amazon Simple Email Service(SES) bounced. **Rohan Deshpande** has a great [article which shows how to handle bounces and complaints](https://aws.amazon.com/blogs/messaging-and-targeting/handling-bounces-and-complaints/). Since this article leans heavily on his approach, I recommend reading that first.
...
-   Set up our dead letter queue to handle messages that could not be handled to help with debugging and isolating problems. Read more about [SQS Dead-Letter Queues](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-dead-letter-queues.html).
...
-   Set the identity to the email domain eg. setting to _example.com_ will set it for emails in the _example.com_ domain.
