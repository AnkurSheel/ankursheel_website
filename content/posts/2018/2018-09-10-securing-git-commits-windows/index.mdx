...
1. Update the global git config to use the signing key. _Remember the Key-ID from above - You will need it here._
...
4. Enter `git config commit.gpgsign true` in your git console. If you want to set it on a global level, use
   `git config --global commit.gpgsign true`instead.

5. Instruct git to sign every tag automatically.

6. Enter `git config tag.gpgsign true` in your git console. If you want to set it on a global level, use
...
2. On the configure page click on _GnuPG System -> Private Keys_ and scroll down to the Options controlling the security
   section.
