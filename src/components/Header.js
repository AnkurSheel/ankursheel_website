...
const HeaderLinks = ({ headerLinks }) => {
    return headerLinks.map((headerLink, i) => (
        <HeaderLink to={headerLink.url} key={`header-link-${i}`} aria-label={`View ${headerLink.label} page`}>
            {headerLink.label}
        </HeaderLink>
    ));
};
...
const BurgerContent = styled.div`
    ...
    position: absolute;
    left: 0;
    ${props => (props.isToggledOn ? 'background: transparent' : `background: ${colors.textLightest}`)};
